...
def modificarCliente(personas):
    ...
    if id in personas:
        ...
        print("Atributos disponibles para modificar: 'nombre', 'edad', 'peso', 'altura', 'sexo', 'objetivo','membresia','estado'")
        print(personas[id])
        atributo = input("Ingrese el atributo que desea modificar: ").lower()
        if atributo in personas[id]:
            nuevo_valor = input(f"Ingrese el nuevo valor para {atributo}: ")
            
            if atributo == "edad" or atributo == "peso" or atributo == "altura" or "membresia" or "estado":
                nuevo_valor = (nuevo_valor)  
            personas[id][atributo] = nuevo_valor  
            
            print(f"El atributo '{atributo}' ha sido modificado.")
            print(personas[id])
        else:
            print(f"El atributo '{atributo}' no existe para el usuario con ID {id}.")
    else:
